...
We will now move on to create a [chloropleth map](https://en.wikipedia.org/wiki/Choropleth_map), which is a type of map commonly used to display election results. Specifically, we will create a chloropleth map of the 2000 electoral college results.
...
ggplot(main_us, aes(geometry = geometry, 
                    fill = winning_party)) +
  geom_sf() + 
...
We can also create an faceted version with the following code:

```{r echo=TRUE, message=FALSE}
#| code-fold: true
main_us <- district_election_2000

# Faceted map with one panel per state
ggplot(main_us, aes(geometry = geometry, 
                    fill = winning_party)) +
  geom_sf() + 
  scale_fill_manual(values = c("Republican" = "red", 
